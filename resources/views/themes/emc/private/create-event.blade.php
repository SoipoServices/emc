        ...
        <form action="{{ route('private.events.store') }}" method="POST" enctype="multipart/form-data" class="max-w-2xl space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div class="tinymce-wrapper">
                    <textarea name="description" id="description" rows="8" required class="block w-full px-3 py-2 text-sm border border-gray-300 rounded-lg  dark:bg-gray-800 dark:border-gray-600 dark:text-white" placeholder="Describe your event in detail...">{{ old('description') }}</textarea>
                </div>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
