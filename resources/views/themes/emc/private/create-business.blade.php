...
<div>
    ...
    <div class="px-4 py-3 border-b border-gray-200 top-16 bg-white/80 dark:bg-black/80 backdrop-blur-md dark:border-gray-800">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-gray-900 dark:text-white">Create an Hustle</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Add a new hustle to the community directory</p>
            </div>
        ...
        <form action="{{ route('private.businesses.store', ['user' => auth()->user()]) }}" method="POST" enctype="multipart/form-data" class="max-w-2xl space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div class="tinymce-wrapper">
                    <textarea name="description" id="description" rows="8" required class="block w-full px-3 py-2 text-sm border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-600 dark:text-white" placeholder="Describe your business in detail...">{{ old('description') }}</textarea>
                </div>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
