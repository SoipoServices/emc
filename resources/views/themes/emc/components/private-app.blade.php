...
<body class="font-sans antialiased">
    ...
    <div class="bg-white dark:bg-black">
        ...
        @impersonating($guard = null)
            <div class="px-4 py-2 text-sm text-center text-white bg-orange-600">
                <div class="flex items-center justify-center gap-2">
                    <x-heroicon-o-finger-print class="w-4 h-4" />
                    <span>You are currently impersonating <strong>{{ auth()->user()->name }}</strong></span>
                    <a href="{{ route('impersonate.leave') }}" class="text-sm text-white underline hover:no-underline">
                        Stop Impersonation
                    </a>
                </div>
            </div>
            ...
            <div class="flex-col hidden h-full pt-4 lg:flex lg:w-54 xl:w-64">
                <nav class="flex-1 px-4 space-y-2">
                    ...
                    @auth
                        ...
                        <a href="{{ route('private.events.list', ['user' => auth()->id()]) }}" class="flex items-center gap-4 px-4 py-3 text-gray-900 transition-colors rounded-full dark:text-white hover:bg-gray-100 dark:hover:bg-gray-900">
                        ...
                        <a href="{{ route('private.library.index', ['user' => auth()->id()]) }}" class="flex items-center gap-4 px-4 py-3 text-gray-900 transition-colors rounded-full dark:text-white hover:bg-gray-100 dark:hover:bg-gray-900">
                            <x-heroicon-s-heart class="w-6 h-6" />
                            <span class="text-xl font-medium">My Library</span>
                        </a>
