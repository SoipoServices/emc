...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request): Response|RedirectResponse
    {
        ...
        if(!empty($category) && !empty($search)){
           $query = User::search($search)->query(fn ($q) => $q->inRandomOrder($request->session()->get('session_rand'))->verified()->isVisible()->hasPosition()->withAnyTags([$category],'categories')->with('tags'));
            ...
            $query = User::inRandomOrder($request->session()->get('session_rand'))->verified()->isVisible()->hasPosition()->with('tags')->withAnyTags([$category],'categories');
        }else{
